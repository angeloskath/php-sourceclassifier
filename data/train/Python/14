...
def won(s):
	s = frozenset(s)
	if s <= frozenset('XT'): return 'X won'
	if s <= frozenset('OT'): return 'O won'
...
def process2(l):
	...
	for col in zip(*l):
		s = won(col)
		if s is not None: return s
	s = won((l[0][0],l[1][1],l[2][2],l[3][3]))
	if s is not None: return s
	s = won((l[0][3],l[1][2],l[2][1],l[3][0]))
	if s is not None: return s
	for row in l:
...
def process(f_, out):
	...
	for X_ in range(1,T_+1):
		input = (f_.readline().strip(),f_.readline().strip(),f_.readline().strip(),f_.readline().strip())
		f_.readline()
